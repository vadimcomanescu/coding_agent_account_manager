...
// TestSyncClaudeConfigFollowsRealHomeKeepsOwnIdentity pins the sync rule: the
// real HOME's configuration wins, the profile's identity and usage cache are
// untouched, and per-project state merges with the real entry winning.
func TestSyncClaudeConfigFollowsRealHomeKeepsOwnIdentity(t *testing.T) {
	realHome := t.TempDir()
	base := filepath.Join(t.TempDir(), "orch-homes")
	if err := os.MkdirAll(filepath.Join(realHome, ".claude"), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(realHome, ".claude.json"), []byte(`{"theme":"dark","oauthAccount":{"accountUuid":"real"},"cachedUsageUtilization":{"accountUuid":"real"},"projects":{"/a":{"allowedTools":["Bash"]},"/shared":{"trust":"real"}}}`), 0o600); err != nil {
		t.Fatal(err)
	}
	mgr, err := NewManager(base, realHome)
	if err != nil {
	...
	if _, err := mgr.Create("p", CreateOptions{Provider: "claude"}); err != nil {
		t.Fatal(err)
	}
	own := `{"theme":"light","editorMode":"vim","oauthAccount":{"accountUuid":"own","emailAddress":"own@example.com"},"cachedUsageUtilization":{"accountUuid":"own"},"projects":{"/b":{"allowedTools":["Read"]},"/shared":{"trust":"own"}}}`
	if err := os.WriteFile(filepath.Join(base, "p", ".claude.json"), []byte(own), 0o600); err != nil {
		t.Fatal(err)
	}

	if err := mgr.SyncClaudeConfig("p"); err != nil {
		t.Fatalf("SyncClaudeConfig: %v", err)
	}
	var got map[string]any
	data, _ := os.ReadFile(filepath.Join(base, "p", ".claude.json"))
	if err := json.Unmarshal(data, &got); err != nil {
		t.Fatal(err)
	}
	if got["theme"] != "dark" {
		t.Errorf("theme = %v, want the real HOME's dark", got["theme"])
	}
	if _, ok := got["editorMode"]; ok {
		t.Errorf("editorMode set only in the shallow lane must not survive a sync (main lane is the source of truth)")
	}
	if got["oauthAccount"].(map[string]any)["accountUuid"] != "own" {
		t.Errorf("identity was overwritten: %v", got["oauthAccount"])
	}
	if got["cachedUsageUtilization"].(map[string]any)["accountUuid"] != "own" {
		t.Errorf("usage cache was overwritten: %v", got["cachedUsageUtilization"])
	}
	projects := got["projects"].(map[string]any)
	if _, ok := projects["/a"]; !ok {
		t.Errorf("real-only project missing")
	}
	if _, ok := projects["/b"]; !ok {
		t.Errorf("shallow-only project dropped")
	}
	if projects["/shared"].(map[string]any)["trust"] != "real" {
		t.Errorf("real entry must win for a project both sides know: %v", projects["/shared"])
	}

	// A profile that never logged in stays identity-free after a sync.
	if _, err := mgr.Create("fresh", CreateOptions{Provider: "claude"}); err != nil {
		t.Fatal(err)
	}
	if err := mgr.SyncClaudeConfig("fresh"); err != nil {
		t.Fatal(err)
	}
	data, _ = os.ReadFile(filepath.Join(base, "fresh", ".claude.json"))
	var fresh map[string]any
	_ = json.Unmarshal(data, &fresh)
	if _, ok := fresh["oauthAccount"]; ok {
		t.Errorf("sync must not hand the real identity to a profile that has none")
	}
}
